import { Position, Board, BoardEvent, Match } from "./board1";

const equalPosition = (p: Position) => (q: Position) =>
  p.row === q.row && p.col === q.col;

const toStringPosition = ({ row, col }: Position) => `(${row}, ${col})`;
const toStringPositions = (ps: Position[]) =>
  `[${ps.map(toStringPosition).join(", ")}]`;
const toStringMatch = <T>({ matched, positions }: Match<T>) =>
  `${toStringPositions(positions)}: ${matched}`;
